...
A tempting shortcut is to Reporting one average score on one dataset encourages the reader to treat a proxy as universal ability and ignores latency, failure recovery, privacy, and misuse. That shortcut fails because it hides the distinction this essay needs to keep visible.
...
**Why the shortcut fails.** A first attempt would answer the question only with word error versus understanding, but that shortcut misses the boundary: A word metric can miss critical entity errors and can penalize harmless orthographic or dialect differences.
...
**Why the shortcut fails.** A first attempt would answer the question only with distribution shift, but that shortcut misses the boundary: A named shift is not evidence of coverage; the shift must be measured and tied to the failure.
...
**Why the shortcut fails.** A first attempt would answer the question only with latency and resource budget, but that shortcut misses the boundary: A faster model may emit less context, reduce quality, or move cost into an unreported service.
...
**Why the shortcut fails.** A first attempt would answer the question only with voice privacy, but that shortcut misses the boundary: Removing words does not necessarily remove speaker identity or sensitive acoustic information.
...
**Why the shortcut fails.** A first attempt would answer the question only with auditability and contestability, but that shortcut misses the boundary: A stored confidence number is not an explanation of what evidence drove the decision.
