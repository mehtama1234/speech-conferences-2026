"""Capture an eight-paper D3 reading set for communication difficulty, identity, dialect, rhythm, style, and contextual ASR."""
import hashlib,json,subprocess,sys,urllib.request
from pathlib import Path
H=Path(__file__).resolve().parent;D=H/"data"
SELECTED={"human-centered-evaluation":"huttner25_interspeech","source-separation-and-spatial-listening":"gan25_interspeech","accent-and-cultural-boundaries":"ducceschi25_interspeech","time-frequency-measurement":"gogoi25_interspeech","text-to-speech-and-content":"kim25b_interspeech","speaker-characteristics":"funk25_interspeech","voice-identity-and-conversion":"chen25d_interspeech","adaptation-and-open-vocabulary":"kong25_interspeech"}
...
(D/"interspeech-2025-thirtyfourth-d3-papers.json").write_text(json.dumps({"venue":"INTERSPEECH 2025","batch_id":"interspeech-2025-d3-batch-034","papers":rows,"failures":[]},indent=2,ensure_ascii=False)+"\n");print(json.dumps({"papers":len(rows)}))
