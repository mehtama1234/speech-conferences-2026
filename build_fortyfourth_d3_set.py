"""Capture an eight-paper D3 reading set for dialogue graphs, fairness, annotation, codec expansion, Arabic TTS, prosody, and audiovisual safety."""
import hashlib,json,subprocess,sys,urllib.request
from pathlib import Path
H=Path(__file__).resolve().parent;D=H/"data"
SELECTED={"dialogue-and-turn-taking":"huang25d_interspeech","source-filter-production":"hutin25_interspeech","privacy-security-and-accountability":"jahan25_interspeech","low-resource-and-data-creation":"johnson25_interspeech","echo-and-reconstruction":"liu25p_interspeech","multilingual-and-crosslingual":"lodagala25_interspeech","prosody-and-interactive-control":"mayer25_interspeech","grounding-and-action":"phukan25f_interspeech"}
...
(D/"interspeech-2025-fortyfourth-d3-papers.json").write_text(json.dumps({"venue":"INTERSPEECH 2025","batch_id":"interspeech-2025-d3-batch-044","papers":rows,"failures":[]},indent=2,ensure_ascii=False)+"\n");print(json.dumps({"papers":len(rows)}))
