"""Capture an eight-paper D3 reading set for time, sensing, alignment, dialogue, and low-resource speech."""
import hashlib,json,subprocess,sys,urllib.request
from pathlib import Path
H=Path(__file__).resolve().parent;D=H/"data"
SELECTED={"time-frequency-measurement":"choi25d_interspeech","room-channel-and-sensing":"ibrahimov25_interspeech","boundaries-and-sequence-structure":"eom25_interspeech","grounding-and-action":"kim25m_interspeech","time-frequency-measurement-2":"ferrofilho25_interspeech","low-resource-and-data-creation":"fong25_interspeech","prosody-and-interactive-control":"lee25f_interspeech","boundaries-and-sequence-structure-2":"hu25e_interspeech"}
...
(D/"interspeech-2025-thirtyfirst-d3-papers.json").write_text(json.dumps({"venue":"INTERSPEECH 2025","batch_id":"interspeech-2025-d3-batch-031","papers":rows,"failures":[]},indent=2,ensure_ascii=False)+"\n");print(json.dumps({"papers":len(rows)}))
