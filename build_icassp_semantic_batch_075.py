"""Review all remaining clearly non-speech ICASSP records in the conservative pool."""
import hashlib, json, re
from pathlib import Path

HERE = Path(__file__).resolve().parent
DATA = HERE / "data"
OUT = DATA / "icassp-2026-semantic-reviewed-batch-075.json"
if OUT.exists():
...
for c in queue["rows"]:
    ...
    selected.append(c)
if len(selected) != 96:
    raise SystemExit(f"expected 96 safe non-speech records after deduplication, found {len(selected)}")
rows = []
for c in selected:
...
payload = {"schema_version": 1, "batch_id": "icassp-2026-semantic-batch-075", "status": "analyst-reviewed-non-speech-boundary-batch", "claim_boundary": "These records are explicit unsupported decisions based on preserved title/abstract evidence; they remain in the full ICASSP denominator.", "reviewed_count": len(rows), "rows": rows}
