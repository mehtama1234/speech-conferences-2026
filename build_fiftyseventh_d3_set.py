...
DATA = HERE / "data"
IDS = [
    "zezario25_interspeech", "onda25b_interspeech", "lay25_interspeech",
    "okabe25_interspeech", "navon25_interspeech", "lu25b_interspeech",
    "lobashev25_interspeech", "yazawa25_interspeech",
]
...
for pid in IDS:
    ...
    if not txt.exists():
        out = subprocess.run([sys.executable, "-c", "import fitz,sys;print('\\n'.join(p.get_text() or '' for p in fitz.open(sys.argv[1])))", str(pdf)], capture_output=True, text=True, timeout=60, check=True)
...
(DATA / "interspeech-2025-fiftyseventh-d3-papers.json").write_text(json.dumps({"venue": "INTERSPEECH 2025", "batch_id": "interspeech-2025-d3-batch-057", "papers": rows, "failures": []}, indent=2, ensure_ascii=False) + "\n")
