...
checks = [
    ...
    {"id":"first-principles-writeups", "status":"verified-with-boundaries" if writeup_audit.get('status') == 'pass' else "in-progress", "evidence":f"The writeup gate finds {writeup_audit.get('complete_subtheme_count', 0)} of {writeup_audit.get('subtheme_count', 0)} subthemes complete against the required plain-language fields. This checks presence and evidence links; human review is still required for conceptual correctness."},
    ...
    {"id":"current-taxonomy-membership-adjudication", "status":"verified-with-boundaries" if taxonomy_adjudication.get('row_count', 0) == 0 else "in-progress", "evidence":f"The current taxonomy has {taxonomy_adjudication.get('row_count', 0)} supported memberships awaiting re-adjudication after concept parents changed. Prior semantic review and mechanical normalization are preserved, but they are not treated as final current-boundary decisions."},
...
payload = {
    ...
    "checks": checks,
    "remaining_work": ["Complete and human-review every theme and subtheme writeup, then confirm or revise each proposed split, merge, and unresolved boundary against the baseline account and named paper evidence before declaring the conceptual taxonomy final."],
