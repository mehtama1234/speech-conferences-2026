"""Capture an eight-paper D3 reading set for clinical phonetics, deepfake tracing, dialogue, coding, translation, and spatial synthesis."""
import hashlib,json,subprocess,sys,urllib.request
from pathlib import Path
H=Path(__file__).resolve().parent;D=H/"data"
SELECTED={"source-filter-production":"chen25n_interspeech","privacy-security-and-accountability":"firc25_interspeech","low-resource-and-data-creation":"glazer25_interspeech","dialogue-and-turn-taking":"gulzar25_interspeech","echo-and-reconstruction":"guo25c_interspeech","multilingual-and-crosslingual":"kponou25_interspeech","grounding-and-action":"kuan25_interspeech","prosody-and-interactive-control":"levkovitch25_interspeech"}
...
(D/"interspeech-2025-thirtyninth-d3-papers.json").write_text(json.dumps({"venue":"INTERSPEECH 2025","batch_id":"interspeech-2025-d3-batch-039","papers":rows,"failures":[]},indent=2,ensure_ascii=False)+"\n");print(json.dumps({"papers":len(rows)}))
