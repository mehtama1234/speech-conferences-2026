#!/usr/bin/env python3
"""Analyst-reject a new bounded batch of clearly out-of-scope ICASSP rows.

The prior rejection scripts were generated from a queue snapshot and can
contain repeated IDs. This batch explicitly excludes every ID in every prior
review batch so its contribution is real unique review coverage.
"""
...
for path in DATA.glob("icassp-2026-semantic-reviewed-batch-*.json"):
    if path.name == "icassp-2026-semantic-reviewed-batch-034.json":
        continue
    payload = json.loads(path.read_text())
    already_reviewed.update(row["paper_id"] for row in payload.get("rows", []))
...
payload = {
    "schema_version": 1,
    "batch_id": "icassp-2026-semantic-batch-034",
...
(DATA / "icassp-2026-semantic-reviewed-batch-034.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n")
