"""Capture an eight-paper D3 reading set for articulation, interaction, and evaluation."""
import hashlib,json,subprocess,sys,urllib.request
from pathlib import Path
H=Path(__file__).resolve().parent;D=H/"data"
SELECTED={"source-filter-production":"bodur25_interspeech","time-frequency-measurement":"bakkouche25b_interspeech","echo-and-reconstruction":"fernandez25_interspeech","dialogue-and-turn-taking":"charuau25_interspeech","metrics-and-targets":"gong25c_interspeech","human-centered-evaluation":"hu25l_interspeech","prosody-and-interactive-control":"janse25_interspeech","adaptation-and-open-vocabulary":"hou25_interspeech"}
...
(D/"interspeech-2025-twentythird-d3-papers.json").write_text(json.dumps({"venue":"INTERSPEECH 2025","batch_id":"interspeech-2025-d3-batch-023","papers":rows,"failures":[]},indent=2,ensure_ascii=False)+"\n");print(json.dumps({"papers":len(rows),"subthemes":len(SELECTED)}))
