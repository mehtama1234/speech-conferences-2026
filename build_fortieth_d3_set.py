"""Capture an eight-paper D3 reading set for articulatory feedback, anti-spoofing, tokens, dialogue, translation, and expressive synthesis."""
import hashlib,json,subprocess,sys,urllib.request
from pathlib import Path
H=Path(__file__).resolve().parent;D=H/"data"
SELECTED={"source-filter-production":"eads25_interspeech","privacy-security-and-accountability":"gorthi25_interspeech","low-resource-and-data-creation":"grigoryan25_interspeech","echo-and-reconstruction":"hartuv25_interspeech","dialogue-and-turn-taking":"hegde25_interspeech","multilingual-and-crosslingual":"kumar25c_interspeech","grounding-and-action":"lee25b_interspeech","prosody-and-interactive-control":"li25i_interspeech"}
...
(D/"interspeech-2025-fortieth-d3-papers.json").write_text(json.dumps({"venue":"INTERSPEECH 2025","batch_id":"interspeech-2025-d3-batch-040","papers":rows,"failures":[]},indent=2,ensure_ascii=False)+"\n");print(json.dumps({"papers":len(rows)}))
