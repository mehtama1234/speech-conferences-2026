...
for paper_id in IDS:
    ...
    rows.append({
        ...
        "full_text_sha256": hashlib.sha256(text_bytes).hexdigest(),
        "full_text_path": str(text.relative_to(HERE)),
        "evidence_depth": "D3",
    })

(DATA / "interspeech-2025-fortyeighth-d3-papers.json").write_text(
    json.dumps({"venue": "INTERSPEECH 2025", "batch_id": "interspeech-2025-d3-batch-048", "papers": rows, "failures": []}, indent=2, ensure_ascii=False) + "\n"
)
