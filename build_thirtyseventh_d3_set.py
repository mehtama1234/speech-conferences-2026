"""Capture an eight-paper D3 reading set for speech data, articulation, emotion, style, language ID, and spoofing."""
import hashlib,json,subprocess,sys,urllib.request
from pathlib import Path
H=Path(__file__).resolve().parent;D=H/"data"
SELECTED={"low-resource-and-data-creation":"arisoy25_interspeech","source-filter-production":"azzouz25_interspeech","echo-and-reconstruction":"chochlakis25_interspeech","prosody-and-interactive-control":"choi25e_interspeech","multilingual-and-crosslingual":"dey25_interspeech","privacy-security-and-accountability":"dutta25b_interspeech","speaker-characteristics":"barahona25_interspeech","grounding-and-action":"bn25_interspeech"}
...
(D/"interspeech-2025-thirtyseventh-d3-papers.json").write_text(json.dumps({"venue":"INTERSPEECH 2025","batch_id":"interspeech-2025-d3-batch-037","papers":rows,"failures":[]},indent=2,ensure_ascii=False)+"\n");print(json.dumps({"papers":len(rows)}))
