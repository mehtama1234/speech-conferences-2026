#!/usr/bin/env python3
"""Reject a deduplicated batch of clearly non-speech ambiguous proposals."""
import hashlib,json,re
from pathlib import Path
HERE=Path(__file__).resolve().parent; DATA=HERE/'data'
pattern=re.compile(r'music|piano|song|singing|phonocardiogram|heart murmur|underwater|ocean acoustic|rf heatmap|sound event|anomalous sound|instrument|ambisonic|chorus|melody|mel-spectrogram',re.I)
...
for path in DATA.glob('icassp-2026-semantic-reviewed-batch-*.json'):
 if path.name!='icassp-2026-semantic-reviewed-batch-042.json': already|={r['paper_id'] for r in json.loads(path.read_text()).get('rows',[])}
